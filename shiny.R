...
bcl <- read.csv('bclData.csv', stringsAsFactors = F) #  Hint: Style rule #5, spaces around "="
...
server <-  function(input, output, session) { #  Hint: Style rule #8, "=" was replaced with "<-"
  print(str(bcl))
  }
...
print(ui) #  Hint: Style rule #5, space between "print" and "(ui)" removed
...
sidebarPanel(sliderInput("priceInput", "Price", 
  min = 0, max = 100, value = c(25, 40), pre = "$"), #  Hint: Style rule #9, semi-colon was removed
...
uiOutput("countryOutput")) #  This helps set up for reactive programming. See 
?uiOutput
...
mainPanel(
  plotOutput("cool.plot"), #  Hint: Style rule #2, instead of an underscore use "."
